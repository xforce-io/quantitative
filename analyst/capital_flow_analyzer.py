...
class CapitalFlowAnalyzer:
    ...
    def analyze_stock_money_flow(self, symbol: str, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"分析个股资金流向失败 {symbol}: {str(e)}")
    ...
    def analyze_northbound_flow(self, symbol: str, days: int = 60) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"分析北向资金关注度失败 {symbol}: {str(e)}")
    ...
    def _get_money_flow_data(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            print(f"获取资金流向数据失败 {symbol}: {str(e)}")
            return pd.DataFrame()
...
def test_capital_flow_analyzer():
    ...
    print("资金流向分析结果:")
    print(f"  主力净流入比例: {flow_result.get('main_inflow_ratio', 'N/A')}")
    print(f"  大单趋势: {flow_result.get('large_order_trend', 'N/A')}")
    print(f"  资金流向一致性: {flow_result.get('flow_consistency', 'N/A')}")
    print(f"  机构关注度: {flow_result.get('institutional_interest', 'N/A')}")
    print(f"  综合评分: {flow_result.get('comprehensive_score', 'N/A')}")

    # 测试北向资金分析
    print(f"\n🌏 测试北向资金分析: {test_symbol}")
    northbound_result = analyzer.analyze_northbound_flow(test_symbol, days=30)
    print("北向资金分析结果:")
    print(f"  关注度评分: {northbound_result.get('northbound_attention', 'N/A')}")
    print(f"  整体趋势: {northbound_result.get('overall_northbound_trend', 'N/A')}")
    print(f"  市场情绪: {northbound_result.get('market_sentiment', 'N/A')}")
