...
class StrategyComparison:
    ...
    def __init__(self, symbol='002594.SZ', start_date='2020-01-01', end_date='2025-01-27'):
        ...
        try:
            ...
            if symbol.startswith('^') or symbol in ['IXIC', 'SPX', 'DJI', 'NDX', 'NASDAQ', 'HSI', 'HKTECH', 'HSCEI']:
                ...
                print(f"🌍 检测到全球指数符号: {symbol}，优先使用Tushare全球指数接口")
            ...
            print(f"警告：数据提供者初始化失败，使用默认设置: {e}")
        ...
        for ma_short, ma_long in ma_combinations:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(f"    ✅ {strategy_name}: {metrics.get('total_return', 'N/A')} (交易{metrics.get('total_trades', 'N/A')}次)")
                else:
        ...
        try:
            ...
            if ts_token:
                ...
                if not df.empty:
                    ...
                    print(f"      ✅ {strategy_name}: {formatted_metrics['total_return']} (交易{formatted_metrics['total_trades']}次)")
                    
                else:
                    print(f"      ❌ 无法获取{self.symbol}的数据")
            else:
                print(f"      ❌ TUSHARE_TOKEN未设置")
        ...
        for period, threshold in momentum_combinations:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    self.results.append(metrics)
                    print(f"    ✅ 收益率: {metrics.get('total_return', 'N/A')}")
                else:
                    print(f"    ❌ 回测失败: {result.stderr}")
        ...
        try:
            ...
            try:
                if self.use_global_index and self.symbol in ['IXIC', 'SPX', 'DJI', 'NDX', 'NASDAQ', 'HSI', 'HKTECH', 'HSCEI']:
                    ...
                    data_type = "全球指数"
                    print(f"🌍 使用Tushare全球指数接口获取 {self.symbol} 数据")
                ...
                if df is None or df.empty:
                    if data_type != "股票":
                        print(f"🔄 {data_type}数据为空，尝试股票接口获取 {self.symbol}")
                        df = self.data_provider.getStockData(
                            self.symbol, start_date_formatted, end_date_formatted, 'D'
                        )
                        if df is not None and not df.empty:
                            print(f"✅ 使用股票接口成功获取 {self.symbol} 数据")
            ...
            print("    🚨 无法获取真实数据，程序退出。不使用模拟数据！")
            print("    💡 请检查数据源连接或使用其他数据提供者")
        ...
        if metric_name in df.columns:
            ...
            try:
                ...
                for i, (_, row) in enumerate(df_sorted.iterrows()):
                    medal = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"][i]
                    print(f"  {medal} {row['策略']}: {row[metric_name]}")
            except Exception as e:
                print(f"    ❌ 排序失败: {str(e)}")
        else:
    ...
    def run_ma_parameter_analysis(self):
        ...
        print(f"📈 分析股票: {self.symbol} - {stock_info['name']} ({stock_info['type']}, {stock_info['volatility']}波动)")
        ...
        for ma_short, ma_long in extended_ma_combinations:
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    print(f"    ✅ MA({ma_short}/{ma_long}): {metrics.get('total_return', 'N/A')}")
                else:
                    print(f"    ❌ MA({ma_short}/{ma_long}): 失败")
                    
            except Exception as e:
                print(f"    ❌ MA({ma_short}/{ma_long}): 异常 - {str(e)}")
    ...
    def _analyze_ma_suitability(self, ma_results, stock_info):
        ...
        if best_ma:
            print(f"\n🏆 最佳MA参数: {best_ma['ma_param']}")
            print(f"   📈 收益率: {best_ma['total_return']}")
            print(f"   📊 夏普比率: {best_ma.get('sharpe_ratio', 'N/A')}")
            print(f"   📉 最大回撤: {best_ma.get('max_drawdown', 'N/A')}")
        ...
        for i, result in enumerate(valid_results[:5]):
            medal = ["🥇", "🥈", "🥉", "4️⃣", "5️⃣"][i]
            print(f"  {medal} {result['ma_param']}: {result['total_return']} (夏普: {result.get('sharpe_ratio', 'N/A')})")
    ...
    def _explain_ma_characteristics(self, ma_short, ma_long, stock_info):
        ...
        print(f"   🎯 类型: {speed_type}")
        print(f"   📝 特点: {characteristic}")
        ...
        if stock_type == '科技股' and volatility == 'high':
            if ma_short <= 10 and ma_long <= 30:
                print(f"   ✅ 优秀匹配: {speed_type}适合高波动科技股")
            else:
                print(f"   ⚠️  一般匹配: 科技股波动较大，建议使用更灵敏的参数")
        elif stock_type == '金融股' and volatility == 'low':
            if ma_short >= 10 and ma_long >= 30:
                print(f"   ✅ 优秀匹配: {speed_type}适合低波动金融股")
            else:
                print(f"   ⚠️  一般匹配: 金融股较稳定，可使用更长周期参数")
        elif stock_type == '资源股' and volatility == 'high':
            if ma_short <= 10 and ma_long <= 20:
                print(f"   ✅ 优秀匹配: {speed_type}适合高波动资源股")
            else:
                print(f"   ⚠️  一般匹配: 资源股需要快速响应价格变化")
        else:
            print(f"   ℹ️  参数与{stock_type}的匹配度需进一步评估")
    ...
    def _give_ma_recommendations(self, stock_info, valid_results):
        ...
        for i, (short, long) in enumerate(recommended_params):
            ...
            priority = ["首选", "备选", "可选"][i]
            print(f"     {i+1}. MA({short}/{long}) - {priority} {performance}")
    
    def run_comparison(self, include_ma_analysis=False, skip_strategies=None):
        """运行完整的策略比较"""
        print("🚀 开始多策略回测比較")
        print(f"📅 测试期间: {self.start_date} ~ {self.end_date}")
