...
from analyst.multi_strategy_comparator import StrategyComparison
...
PORTFOLIOS = portfolio_manager.portfolios
...
class InvestmentAdvisor:
    ...
    def _analyze_current_signal(self, symbol: str, days: int = 300) -> dict:
        ...
        # 计算包含资金流的综合评分
        combined_score = signal_score + canslim_score + capital_flow_score
    ...
    def _generate_action_advice(self, current_signal: dict) -> str:
        ...
        # 如果有CANSLIM分析结果，显示详细信息
        if 'canslim_analysis' in current_signal:
            canslim_assessment = current_signal['canslim_analysis'].get('overall_assessment', '')
            return f"{action}（综合评分 {combined_score}：技术面 {technical_score}，基本面 {canslim_score}，{canslim_assessment}）"
        else:
    ...
    def analyze_current_signals_only(self, symbols=None, current_days: int = 300):
        ...
        for symbol, data in results.items():
            if 'error' not in data and data.get('current_signal'):
                ...
                print(f"  📈 综合评分: {signal.get('combined_score', 0)} (技术: {signal.get('technical_score', 0)}, 基本面: {signal.get('canslim_score', 0)})")
