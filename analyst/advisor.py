...
# 导入两个advisor
from analyst.smart_advisor import IntegratedAdvisor as SmartAdvisor
from analyst.investment_advisor import InvestmentAdvisor
...
class UnifiedAdvisor:
    ...
    def analyze_portfolio(self, symbols: List[str] = None, portfolio_name: str = 'CUSTOM', weights: Optional[Dict[str, float]] = None, use_sleeves: bool = False, **kwargs) -> Dict[str, Any]:
        ...
        if not symbols:
            try:
                ...
                print(f"📚 使用预定义组合: {portfolio_name}")
                ...
                if sleeves and use_sleeves:
                    ...
                    for sleeve in sleeves:
                        print(f"  • {sleeve.name} ({sleeve.risk_level}): {sleeve.allocation_ratio:.1%} 配置, 目标收益 {sleeve.target_return:.1%}")
                    # 使用sleeve配置的权重
                ...
                print(f"❌ {e}")
                print(f"📊 可用组合: {', '.join(available_portfolios)}")
                ...
                try:
                    ...
                    if advisor.data is not None and len(advisor.data) > 20:
                        ...
                        if 'error' not in etf_flow_result:
                            print(f"✅ ETF资金流分析完成 - 综合评分: {etf_flow_result['comprehensive_score']}/100")
                        else:
                            print(f"⚠️ ETF资金流分析数据不足: {etf_flow_result['error']}")
                    else:
                        print(f"⚠️ 无法获取{symbol}的日线数据")
                        etf_flow_result = {'error': '无法获取日线数据'}
                        
                except Exception as etf_e:
                    print(f"⚠️ ETF资金流分析异常: {etf_e}")
    ...
    def analyze_all_portfolios(self, analysis_mode: str = 'portfolio', **kwargs) -> Dict[str, Any]:
        ...
        available_portfolios = self.portfolio_manager.list_portfolios()
        print(f"📊 组合数量: {len(available_portfolios)}")
        print("=" * 80)
            ...
            try:
                if analysis_mode == 'portfolio':
                    ...
                    for j, symbol in enumerate(portfolio_symbols[:5]):  # 限制前5个标的避免太长
                        print(f"  🎯 分析标的 {j+1}/{min(5, len(portfolio_symbols))}: {symbol}")
                ...
                if result.get('success', True):
                    successful_count += 1
                    print(f"✅ {portfolio_name} 分析完成")
                else:
                    failed_count += 1
                    print(f"❌ {portfolio_name} 分析失败")
        ...
        print(f"\n" + "=" * 80)
        print(f"📋 全组合分析完成汇总")
        print(f"📊 总组合数: {len(available_portfolios)}")
        print(f"✅ 成功: {successful_count}")
        print(f"❌ 失败: {failed_count}")
        print(f"📈 成功率: {successful_count/len(available_portfolios)*100:.1f}%")
        
        if global_summary['global_insights']:
            insights = global_summary['global_insights']
            print(f"\n🎯 全局洞察:")
            if insights.get('top_performing_portfolios'):
                print(f"🏆 表现最佳组合: {', '.join([p['name'] for p in insights['top_performing_portfolios'][:3]])}")
            if insights.get('sector_distribution'):
                print(f"📊 行业分布: {insights['sector_distribution']}")
    ...
    def analyze_screen(self, screen_name: str, weight_method: str = 'equal', use_proxy: bool = True, current_days: int = 300, use_dynamic: bool = False, auto_refresh: bool = False, **kwargs) -> Dict[str, Any]:
        ...
        print(f"📚 使用{screen_type}选股篮子: {screen_name} | 权重: {weight_method}")
        ...
        if use_dynamic and auto_refresh:
            try:
                ...
                if refresh_success:
                    print(f"✅ 动态篮子 {screen_name} 刷新成功")
                else:
                    print(f"⚠️ 动态篮子 {screen_name} 刷新失败，使用现有数据")
            except Exception as e:
                print(f"⚠️ 动态篮子刷新异常: {e}")
    ...
    def refresh_dynamic_screens(self, screen_names: List[str] = None, force: bool = False) -> Dict[str, Any]:
        ...
        print(f"✅ 动态篮子刷新完成: {success_count}/{total_count} 成功")
            ...
            for line in f:
                ...
                if line and not line.startswith('#'):
                    ...
                    if ',' in line:
                        ...
                        symbols.append(line)
        print(f"📄 从文件加载了 {len(symbols)} 个股票代码")
    except Exception as e:
        print(f"❌ 加载文件失败: {e}")
