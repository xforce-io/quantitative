...
class StrategyPerformanceAnalyzer:
            ...
            if experiment_dir.is_dir():
                ...
                if results_file.exists():
                    try:
                        ...
                        logger.info("✅ 加载实验: {experiment_dir.name} ({len(data)} 个策略)")
                        
                    except Exception as e:
                        logger.info("❌ 加载失败 {experiment_dir.name}: {e}")
        
        logger.info("📈 总共加载了 {len(self.results)} 个策略结果")
    ...
    def createDataFrame(self) -> None:
        ...
        logger.info("✅ 创建DataFrame: {self.df.shape[0]} 行, {self.df.shape[1]} 列")
    ...
    def correlationAnalysis(self) -> pd.DataFrame:
        ...
        for feature, corr in return_correlations.items():
            if feature != 'total_return':
                ...
                logger.info("{feature:<20} | {corr:>6.3f} | {strength} {direction}")
    ...
    def parameterImpactAnalysis(self) -> Dict:
        ...
        for param, analysis in impact_analysis.items():
            logger.info("\n{param.upper()}:")
            logger.info("  与总收益率相关性: {analysis['correlation']:.3f}")
            if 'groups' in analysis:
                logger.info("  分组平均收益:")
                for group, avg_return in analysis['groups'].items():
                    logger.info("    {group}: {avg_return:.2%}")
    ...
    def generateVisualizations(self, output_dir: str = "reports/strategy_analysis") -> None:
        ...
        logger.info("📊 可视化图表已保存到: {output_path}")
...
"""
        ...
        logger.info("📄 分析报告已保存到: {output_path}")

def main():
    """主函数"""
    logger.info("🚀 策略性能分析工具启动")
    print("=" * 60)
    ...
    logger.info("\n✅ 分析完成！")
    logger.info("📊 可视化图表: reports/strategy_analysis/")
    logger.info("📄 详细报告: reports/strategy_analysis_report.md")
