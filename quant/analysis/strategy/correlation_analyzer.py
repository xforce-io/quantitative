...
class StrategyCorrelationAnalyzer:
    ...
    def __init__(self, symbols: List[str] = None):
        ...
        logger.info("   📊 策略配置数量: {len(self.strategy_configs)}")
        logger.info("   📈 测试周期数量: {len(self.backtest_periods)}")
        logger.info("   🏷️  测试股票: {', '.join(self.symbols)}")
    ...
    def _define_strategy_configs(self) -> List[StrategyConfig]:
        ...
        logger.info("📋 生成策略配置:")
        logger.info("   MA交叉: {len(ma_combinations) * len(volume_filters)} 组合")
        logger.info("   动量策略: {len(momentum_combinations)} 组合") 
        logger.info("   网格策略: {len(grid_combinations)} 组合")
        logger.info("   塞柯塔MA: {len(seykota_combinations)} 组合")
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_task):
                ...
                try:
                    ...
                    if result.success:
                        completed_tasks += 1
                        logger.info("✅ [{completed_tasks}/{total_tasks}] {result.strategy_config.name} | {symbol} | {period.name}: {result.total_return:.2%}")
                    else:
                        failed_tasks += 1
                        logger.info("❌ [{completed_tasks + failed_tasks}/{total_tasks}] {result.strategy_config.name} | {symbol} | {period.name}: {result.error_message}")
                        
                except Exception as e:
                    failed_tasks += 1
                    logger.info("❌ [{completed_tasks + failed_tasks}/{total_tasks}] {strategy_config.name} | {symbol} | {period.name}: 执行异常 - {str(e)}")
        
        logger.info("\n📊 回测完成统计:")
        logger.info("   ✅ 成功: {completed_tasks}")
        logger.info("   ❌ 失败: {failed_tasks}")
        logger.info("   📈 成功率: {completed_tasks/(completed_tasks+failed_tasks):.1%}")
    ...
    def _get_stock_data(self, symbol: str, start_date: str, end_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            logger.info("获取{symbol}数据失败: {str(e)}")
            return pd.DataFrame()
    
    def calculate_correlation_matrix(self) -> pd.DataFrame:
        """计算策略相关性矩阵"""
        print("\n" + "="*60)
        logger.info("📊 计算策略相关性矩阵")
        print("="*60)
        ...
        if strategy_returns:
            ...
            logger.info("✅ 相关性矩阵计算完成: {correlation_matrix.shape}")
            logger.info("   包含策略组合: {len(correlation_matrix)}")
            
            return correlation_matrix
        
        else:
            logger.info("❌ 没有有效的收益率数据")
            return pd.DataFrame()
    
    def analyze_correlations(self) -> Dict[str, Any]:
        """分析策略相关性"""
        if self.correlation_matrix is None or self.correlation_matrix.empty:
            logger.info("❌ 请先计算相关性矩阵")
            return {}
        ...
        # 打印分析结果
        logger.info("\n📈 相关性统计:")
        logger.info("   平均相关性: {analysis_result['matrix_summary']['mean_correlation']:.3f}")
        logger.info("   最高相关性: {analysis_result['matrix_summary']['max_correlation']:.3f}")
        logger.info("   最低相关性: {analysis_result['matrix_summary']['min_correlation']:.3f}")
        
        logger.info("\n🔴 高相关性策略对 (>0.8): {len(high_corr_pairs)}")
        for pair in high_corr_pairs[:5]:  # 显示前5个
            logger.info("   📊 {pair['correlation']:.3f}: {pair['strategy1'][:30]} <-> {pair['strategy2'][:30]}")
    ...
    def generate_comprehensive_report(self, output_dir: str = "reports/strategy_correlation_analysis") -> None:
        ...
        logger.info("✅ 报告生成完成，保存至: {output_path.absolute()}")
