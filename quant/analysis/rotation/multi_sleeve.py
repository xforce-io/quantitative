...
Frequency-aware strategy-level rankers used by the published SOTA rotation
baseline (config/strategies/rotation/sota.json):
...
Discovery/research scripts and live advisor import from here.
"""
...
class MultiSleeveRanker:
    ...
    def __init__(
        ...
        risk_on_rule: dict | None = None,
        risk_on_allocation: dict | None = None,
        risk_off_allocation: dict | None = None,
        volumes: "pd.DataFrame | None" = None,
        frequency: BacktestFrequency = "monthly",
        *,
        monthly_benchmark: pd.Series | None = None,
        monthly_volumes: "pd.DataFrame | None" = None,
        sleeve_map: dict[str, list[str]] | None = None,
        factor_store=None,
        symbol_aliases: dict[str, str] | None = None,
    ) -> None:
        # Backward-compatible aliases used by older call sites / live_advisor.
        if monthly_benchmark is not None:
            benchmark = monthly_benchmark
        if benchmark is None:
            raise TypeError("benchmark (or monthly_benchmark) is required")
    ...
    def rank(self, prices: pd.DataFrame, rebalance_date: pd.Timestamp) -> dict[str, float]:
        ...
        if is_risk_on:
            ...
            symbols = self.sleeve_map.get(sleeve, list(prices.columns))
            ...
            if _cb_cfg.get("enabled") and self._cb_forced_hold > 0:
                ...
                _fb = self.symbol_aliases.get(_fb_raw, _fb_raw)
            ...
            if candidates_cfg:
                candidate_syms = [self.symbol_aliases.get(c.get("real_etf", ""), c.get("real_etf", ""))
        ...
        if is_risk_on:
            ...
            if not result:
                ...
                if fallback_etf:
                    symbol = self.symbol_aliases.get(fallback_etf, fallback_etf)
        ...
        if not result:
            ...
            if floor_cfg.get("enabled"):
                ...
                fallback_sym = self.symbol_aliases.get(fallback, fallback)
                ...
                if not pd.isna(curr_p) and curr_p > 0:
                    ...
                    if self._cb_peak_price and self._cb_peak_price > 0:
                        ...
                        if drawdown < cb_thr:
                            ...
                            fb = self.symbol_aliases.get(fb_raw, fb_raw)
...
def build_ranker_from_spec(
    ...
    *,
    ...
    if monthly_benchmark is not None:
        benchmark = monthly_benchmark
    if benchmark is None:
        raise ValueError("benchmark (or monthly_benchmark) is required")
    if monthly_volumes is not None:
        volumes = monthly_volumes
